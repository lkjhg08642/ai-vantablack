...
void forwardStraight(double fdistance) {
    ...
    while (fabs(currentPos) < fabs(distanceEncoder)) {
        ...
        if ((currentAngle - angle) > AngleTolerance) {
            ...
            leftDriveSmart.spin(vex::directionType::fwd,  dir * instantV * percentChange, vex::velocityUnits::rpm);
            rightDriveSmart.spin(vex::directionType::fwd, dir * instantV,                 vex::velocityUnits::rpm);
        } else if ((currentAngle - angle) < (-AngleTolerance)) {
            // turn left a little bit
            leftDriveSmart.spin(vex::directionType::fwd,  dir * instantV,                 vex::velocityUnits::rpm);
            rightDriveSmart.spin(vex::directionType::fwd, dir * instantV * percentChange, vex::velocityUnits::rpm);
        } else {
            leftDriveSmart.spin(vex::directionType::fwd,  dir * instantV, vex::velocityUnits::rpm);
            rightDriveSmart.spin(vex::directionType::fwd, dir * instantV, vex::velocityUnits::rpm);
        }
...
void teleop(void) {
  ...
  while (1) {
    ...
    leftDriveSmart.spin(vex::directionType::fwd, flSpeed, percent);
    rightDriveSmart.spin(vex::directionType::fwd, brSpeed, percent);
